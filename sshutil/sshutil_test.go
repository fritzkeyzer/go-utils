...
import (
	...
	"github.com/fritzkeyzer/go-utils/sshutil"
)
...
//	t.Run("wrongPassTest", wrongPassTest)
//}

func authTest(t *testing.T) {
	newServer("2020")

	_, err := sshutil.NewConn(&sshutil.Config{
		Addr:     "127.0.10.10",
		Port:     2020,
		User:     "melbahja",
		Auth:     sshutil.Password("123456"),
...
func runTest(t *testing.T) {
	...
	client, err := sshutil.NewConn(&sshutil.Config{
		Addr:     "127.0.10.10",
		Port:     2021,
		User:     "melbahja",
		Auth:     sshutil.Password("123456"),
...
func wrongPassTest(t *testing.T) {
	newServer("2022")
...
func newServer(port string) {
	...
	go func() {
		...
		for newChannel := range chans {
			...
			termi := term.NewTerminal(channel, "> ")

			go func() {
				defer channel.Close()
				for {
					line, err := termi.ReadLine()
					if err != nil {
						break
					}
					fmt.Println(line)
